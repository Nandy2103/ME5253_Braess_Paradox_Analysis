function braess_paradox_topology()
    ...
    % Number of nodes
    A_full = ones(8) - eye(8);  % All ones except diagonal zeros
    K_full = K_uniform*A_full;

     % --- Network 2: "Add capacity" (Figure 1b) ---
    % Doubles capacity of edge (3,4)
    A_add_capacity = A_full; % Topology is the same
    K_matrix_add_capacity = K_uniform * A_add_capacity; % Start with uniform couplings
    K_matrix_add_capacity(4, 8) = 0 * K_uniform; % Double K for edge (3,4)
    K_matrix_add_capacity(8, 4) = 0 * K_uniform; % Symmetric

    %Star Topology
    A_star = zeros(N, N);
    for n = 2:N
        A_star(1, n) = 1;
        A_star(n, 1) = 1; % Undirected connection
    end
    K_star = K_uniform * A_star;

    %Ring Topology
    A_ring = zeros(N, N);
    for n = 1:N
        n1 = n;
        n2 = mod(n, N) + 1; % Connects 1->2, 2->3, ..., 7->8, and 8->1
        
        A_ring(n1, n2) = 1;
        A_ring(n2, n1) = 1; % Undirected connection
    end
    K_ring = K_uniform * A_ring;
    ...
    x_coords = [6.0, 5.0, 6.0, 3.0, 2.0, 3.0, 7.0, 4.0]; % Corrected x-coordinates
    y_coords = [2.0, 3.0, 4.0, 4.0, 3.0, 2.0, 3.0, 3.0]; % Corrected y-coordinates
    ...
    G_full = graph(A_full, node_labels, 'omitselfloops');
    G_star = graph(A_star, node_labels, 'omitselfloops');
    G_ring = graph(A_ring, node_labels, 'omitselfloops');
    ...
    % Subplot 2: Fully Connected
    % subplot(1, 4, 2);
    % p2 = plot(G_full, 'XData', x_coords, 'YData', y_coords, 'LineWidth', 2);
    % title('b Full');
    % customize_plot(p2, producers, consumers, [], []);

    subplot(1, 4, 2);
    p2 = plot(G_add_capacity, 'XData', x_coords, 'YData', y_coords, 'LineWidth', 2);
    title('b Add capacity');
    customize_plot(p2, producers, consumers, [8,4], [0.1 0.1 0.9]); % Highlight edge (3,4) in blue

    % Subplot 3: Star Connected
    subplot(1, 4, 3);
    p2 = plot(G_star, 'XData', x_coords, 'YData', y_coords, 'LineWidth', 2);
    title('c Star');
    customize_plot(p2, producers, consumers, [], []);

    % Subplot 4: Ring Connected
    subplot(1, 4, 4);
    p2 = plot(G_ring, 'XData', x_coords, 'YData', y_coords, 'LineWidth', 2);
    title('d Ring');
    customize_plot(p2, producers, consumers, [], []);
    ...
    [t_full, y_full] = ode45(@(t, y) second_order_ode(t, y, P_vec, K_full, alpha), tspan, y0, options);
    [t_star, y_star] = ode45(@(t, y) second_order_ode(t, y, P_vec, K_star, alpha), tspan, y0, options);
    [t_ring, y_ring] = ode45(@(t, y) second_order_ode(t, y, P_vec, K_ring, alpha), tspan, y0, options);
    ...
    phi_pi_full = y_full(:, 1:N) / pi;
    phi_pi_cap = y_cap(:, 1:N) / pi;
    phi_pi_star = y_star(:, 1:N) / pi;
    phi_pi_ring = y_ring(:, 1:N) / pi;
    ...
    % h_cap_prod = plot(t_full, phi_pi_full(:, producers), 'r-', 'LineWidth', 1.5);
    % h_cap_cons = plot(t_full, phi_pi_full(:, consumers), 'g-', 'LineWidth', 1.5);
    hold off;
    xlabel('\alpha t');
    ylabel('\phi_j(t) / \pi');
    grid on;
    ...
    subplot(1, 4, 3);
    hold on;
    h_cap_prod = plot(t_star, phi_pi_star(:, producers), 'r-', 'LineWidth', 1.5);
    h_cap_cons = plot(t_star, phi_pi_star(:, consumers), 'g-', 'LineWidth', 1.5);
    hold off;
    xlabel('\alpha t');
    ylabel('\phi_j(t) / \pi');
    grid on;
    ...
    subplot(1, 4, 4);
    hold on;
    h_cap_prod = plot(t_ring, phi_pi_ring(:, producers), 'r-', 'LineWidth', 1.5);
    h_cap_cons = plot(t_ring, phi_pi_ring(:, consumers), 'g-', 'LineWidth', 1.5);
    hold off;
    xlabel('\alpha t');
    ylabel('\phi_j(t) / \pi');
    grid on;
    ...
    K_list = linspace(0.0, 2, 25) * P_base;
    r_orig = zeros(size(K_list));
    r_full  = zeros(size(K_list));
    r_cap  = zeros(size(K_list));
    r_star = zeros(size(K_list));
    r_ring = zeros(size(K_list));
    ...
    for k = 1:length(K_list)
        ...
        Kmat = K_list(k) * A_full;
        [~, y] = ode45(@(t,y) second_order_ode(t,y,P_vec,Kmat,alpha), [0 40], y0);
        phi = y(end,1:N);
        r_full(k) = abs(mean(exp(1i*phi)));
        ...
        Kmat = K_list(k) * A_star;
        [~, y] = ode45(@(t,y) second_order_ode(t,y,P_vec,Kmat,alpha), [0 40], y0);
        phi = y(end,1:N);
        r_star(k) = abs(mean(exp(1i*phi)));

        Kmat = K_list(k) * A_ring;
        [~, y] = ode45(@(t,y) second_order_ode(t,y,P_vec,Kmat,alpha), [0 40], y0);
        phi = y(end,1:N);
        r_ring(k) = abs(mean(exp(1i*phi)));
    end
    ...
    plot(K_list/P_base, r_full,  'b--', 'LineWidth',2);
    plot(K_list/P_base, r_cap,  'b--', 'LineWidth',2);
    plot(K_list/P_base, r_star,  'g--', 'LineWidth',2);
    plot(K_list/P_base, r_ring,  'r--', 'LineWidth',2);
    xlabel('K/P'); ylabel('r (order parameter)');
    legend('Original','full sys','Cap','Star','Ring','Location','southeast');
